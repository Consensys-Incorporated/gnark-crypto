...
type TestCases struct {
	Inputs  []mamabear.Element `json:"inputs"`
	Entries []struct {
		Params struct {
		...
		Expected []mamabear.Element `json:"expected"`
	} `json:"entries"`
}
...
func TestReference(t *testing.T) {
	...
	for testCaseID, testCase := range testCases.Entries {
		...
		if testCase.Params.LogTwoBound > mamabear.Bits {
			t.Logf("skipping test case %d, logTwoBound %d is greater than field bit size (%d)", testCaseID, testCase.Params.LogTwoBound, mamabear.Bits)
			continue
		}
		...
		goHash := make([]mamabear.Element, 1<<testCase.Params.LogTwoDegree)
...
func TestLimbDecomposeBytes(t *testing.T) {
	...
	// the Montgomery radix, narrower here than the storage word
	bMontConstant.Lsh(&bMontConstant, 52)
	...
	for range 3 {
		...
		coeffsPerFieldsElmt := mamabear.Bytes * 8 / logTwoBound
		for i := range nbElmts {
