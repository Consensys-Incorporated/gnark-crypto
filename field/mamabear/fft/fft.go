...
func kerDIFNP_32generic(a []mamabear.Element, twiddles [][]mamabear.Element, stage int) {
	...
	for offset := 0; offset < 32; offset += 2 {
		mamabear.Butterfly(&a[offset], &a[offset+1])
	}
}

func kerDITNP_32generic(a []mamabear.Element, twiddles [][]mamabear.Element, stage int) {
	// code unrolled & generated by internal/generator/fft/template/fft.go.tmpl
...
func kerDIFNP_64(a []mamabear.Element, twiddles [][]mamabear.Element, stage int) {
	...
	// Stages 2-4 are inlined to avoid function call and Vector dispatch overhead.
	{
		...
		for offset := 0; offset < 64; offset += 32 {
			...
			mamabear.Butterfly(&a[o1], &a[o1+8])
			mamabear.Butterfly(&a[o2], &a[o2+8])
			for i := 1; i < 8; i++ {
				mamabear.Butterfly(&a[o1+i], &a[o1+i+8])
				mamabear.Butterfly(&a[o2+i], &a[o2+i+8])
			}
			for i := 1; i < 8; i++ {
	...
	}
	{
		...
		for offset := 0; offset < 64; offset += 16 {
			...
			mamabear.Butterfly(&a[o1], &a[o1+4])
			mamabear.Butterfly(&a[o2], &a[o2+4])
			for i := 1; i < 4; i++ {
				mamabear.Butterfly(&a[o1+i], &a[o1+i+4])
				mamabear.Butterfly(&a[o2+i], &a[o2+i+4])
			}
			for i := 1; i < 4; i++ {
	...
	}
	{
		...
		for offset := 0; offset < 64; offset += 4 {
			mamabear.Butterfly(&a[offset], &a[offset+2])
			mamabear.Butterfly(&a[offset+1], &a[offset+3])
