// Copyright 2020-2026 Consensys Software Inc.
// Licensed under the Apache License, Version 2.0. See the LICENSE file for details.

package fft

import (
	"encoding/binary"
	"fmt"
	"math/big"
	"math/rand/v2"
	"strconv"
	"testing"

	"github.com/consensys/gnark-crypto/field/mamabear"
	"github.com/consensys/gnark-crypto/utils"

	"github.com/leanovate/gopter"
	"github.com/leanovate/gopter/gen"
	"github.com/leanovate/gopter/prop"

	"github.com/stretchr/testify/require"
)

func TestFFT(t *testing.T) {
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 6
	properties := gopter.NewProperties(parameters)

	for maxSize := 2; maxSize <= 1<<10; maxSize <<= 1 {

		domainWithPrecompute := NewDomain(uint64(maxSize))
		domainWithoutPrecompute := NewDomain(uint64(maxSize), WithoutPrecompute())

		for domainName, domain := range map[string]*Domain{
			"with precompute":    domainWithPrecompute,
			"without precompute": domainWithoutPrecompute,
		} {
			t.Logf("domain: %s", domainName)
			properties.Property("DIF FFT should be consistent with dual basis", prop.ForAll(

				func(ithpower int) bool {

					pol := make([]mamabear.Element, maxSize)
					backupPol := make([]mamabear.Element, maxSize)

					for i := range maxSize {
						pol[i].MustSetRandom()
					}
					copy(backupPol, pol)

					domain.FFT(pol, DIF)
					utils.BitReverse(pol)

					sample := domain.Generator
					sample.Exp(sample, big.NewInt(int64(ithpower)))

					eval := evaluatePolynomial(backupPol, sample)

					return eval.Equal(&pol[ithpower])

				},
				gen.IntRange(0, maxSize-1),
			))

			properties.Property("DIF FFT on cosets should be consistent with dual basis", prop.ForAll(

				func(ithpower int) bool {

					pol := make([]mamabear.Element, maxSize)
					backupPol := make([]mamabear.Element, maxSize)

					for i := range maxSize {
						pol[i].MustSetRandom()
					}
					copy(backupPol, pol)

					domain.FFT(pol, DIF, OnCoset())
					utils.BitReverse(pol)

					sample := domain.Generator
					sample.Exp(sample, big.NewInt(int64(ithpower))).
						Mul(&sample, &domain.FrMultiplicativeGen)

					eval := evaluatePolynomial(backupPol, sample)

					return eval.Equal(&pol[ithpower])

				},
				gen.IntRange(0, maxSize-1),
			))

			properties.Property("DIT FFT should be consistent with dual basis", prop.ForAll(

				func(ithpower int) bool {

					pol := make([]mamabear.Element, maxSize)
					backupPol := make([]mamabear.Element, maxSize)

					for i := range maxSize {
						pol[i].MustSetRandom()
					}
					copy(backupPol, pol)

					utils.BitReverse(pol)
					domain.FFT(pol, DIT)

					sample := domain.Generator
					sample.Exp(sample, big.NewInt(int64(ithpower)))

					eval := evaluatePolynomial(backupPol, sample)

					return eval.Equal(&pol[ithpower])

				},
				gen.IntRange(0, maxSize-1),
			))

			properties.Property("BitReverse(DIF FFT(DIT FFT (BitReverse))))==id", prop.ForAll(

				func() bool {

					pol := make([]mamabear.Element, maxSize)
					backupPol := make([]mamabear.Element, maxSize)

					for i := range maxSize {
						pol[i].MustSetRandom()
					}
					copy(backupPol, pol)

					utils.BitReverse(pol)
					domain.FFT(pol, DIT)
					domain.FFTInverse(pol, DIF)
					utils.BitReverse(pol)

					check := true
					for i := range len(pol) {
						check = check && pol[i].Equal(&backupPol[i])
					}
					return check
				},
			))

			for nbCosets := 2; nbCosets < 5; nbCosets++ {
				properties.Property(fmt.Sprintf("BitReverse(DIF FFT(DIT FFT (BitReverse))))==id on %d cosets", nbCosets), prop.ForAll(

					func() bool {

						pol := make([]mamabear.Element, maxSize)
						backupPol := make([]mamabear.Element, maxSize)

						for i := range maxSize {
							pol[i].MustSetRandom()
						}
						copy(backupPol, pol)

						check := true

						for i := 1; i <= nbCosets; i++ {

							utils.BitReverse(pol)
							domain.FFT(pol, DIT, OnCoset())
							domain.FFTInverse(pol, DIF, OnCoset())
							utils.BitReverse(pol)

							for i := range len(pol) {
								check = check && pol[i].Equal(&backupPol[i])
							}
						}

						return check
					},
				))
			}

			properties.Property("DIT FFT(DIF FFT)==id", prop.ForAll(

				func() bool {

					pol := make([]mamabear.Element, maxSize)
					backupPol := make([]mamabear.Element, maxSize)

					for i := range maxSize {
						pol[i].MustSetRandom()
					}
					copy(backupPol, pol)

					domain.FFTInverse(pol, DIF)
					domain.FFT(pol, DIT)

					check := true
					for i := range len(pol) {
						check = check && (pol[i] == backupPol[i])
					}
					return check
				},
			))

			properties.Property("DIT FFT(DIF FFT)==id on cosets", prop.ForAll(

				func() bool {

					pol := make([]mamabear.Element, maxSize)
					backupPol := make([]mamabear.Element, maxSize)

					for i := range maxSize {
						pol[i].MustSetRandom()
					}
					copy(backupPol, pol)

					domain.FFTInverse(pol, DIF, OnCoset())
					domain.FFT(pol, DIT, OnCoset())

					for i := range len(pol) {
						if !(pol[i].Equal(&backupPol[i])) {
							return false
						}
					}

					// compute with nbTasks == 1
					domain.FFTInverse(pol, DIF, OnCoset(), WithNbTasks(1))
					domain.FFT(pol, DIT, OnCoset(), WithNbTasks(1))

					for i := range len(pol) {
						if !(pol[i].Equal(&backupPol[i])) {
							return false
						}
					}

					return true
				},
			))
		}
		properties.TestingRun(t, gopter.ConsoleReporter(false))
	}
}

// mamabear prime p = 2^49 − 2^34 + 1
const mamabearP = uint64(562932773552129)

func randElement(rng *rand.Rand) mamabear.Element {
	return mamabear.Element{rng.Uint64N(mamabearP)}
}

func FuzzFFT(f *testing.F) {
	f.Fuzz(func(t *testing.T, domainSize uint16, rngSeed int64) {
		if domainSize > (1 << 13) {
			t.Skip("domain size too large")
		}
		if domainSize < 2 {
			t.Skip("domain size too small")
		}

		domain := NewDomain(uint64(domainSize))

		var seed [32]byte
		binary.PutVarint(seed[:], rngSeed)
		// #nosec G404 -- fuzz does not require a cryptographic PRNG
		rng := rand.New(rand.NewChaCha8(seed))

		cardinality := domain.Cardinality

		a, b := make([]mamabear.Element, cardinality), make([]mamabear.Element, cardinality)
		for i := range int(cardinality) {
			a[i] = randElement(rng)
		}
		copy(b, a)

		domain.FFTInverse(a, DIF)
		domain.FFT(a, DIT)

		assert := require.New(t)
		for i := range int(cardinality) {
			assert.True(a[i].Equal(&b[i]), "FFT-1(FFT(pol)) != pol at index %d", i)
		}
	})
}

// --------------------------------------------------------------------
// benches

func BenchmarkFFT(b *testing.B) {

	const maxSize = 1 << 20

	pol := make([]mamabear.Element, maxSize)
	pol[0].MustSetRandom()
	for i := 1; i < maxSize; i++ {
		pol[i] = pol[i-1]
	}

	for i := 8; i < 20; i++ {
		sizeDomain := 1 << i
		b.Run("fft 2**"+strconv.Itoa(i)+"bits", func(b *testing.B) {
			domain := NewDomain(uint64(sizeDomain))
			b.ResetTimer()
			for range b.N {
				domain.FFT(pol[:sizeDomain], DIT)
			}
		})
		b.Run("fft 2**"+strconv.Itoa(i)+"bits (coset)", func(b *testing.B) {
			domain := NewDomain(uint64(sizeDomain))
			b.ResetTimer()
			for range b.N {
				domain.FFT(pol[:sizeDomain], DIT, OnCoset())
			}
		})
	}
}

func BenchmarkFFTDITCosetReference(b *testing.B) {
	const maxSize = 1 << 20

	pol := make([]mamabear.Element, maxSize)
	pol[0].MustSetRandom()
	for i := 1; i < maxSize; i++ {
		pol[i] = pol[i-1]
	}

	domain := NewDomain(maxSize)

	b.ResetTimer()
	for range b.N {
		domain.FFT(pol, DIT, OnCoset())
	}
}

func BenchmarkFFTDITReferenceSmall(b *testing.B) {
	const maxSize = 1 << 9

	pol := make([]mamabear.Element, maxSize)
	pol[0].MustSetRandom()
	for i := 1; i < maxSize; i++ {
		pol[i] = pol[i-1]
	}

	domain := NewDomain(maxSize)

	b.ResetTimer()
	for range b.N {
		domain.FFT(pol, DIT)
	}
}

func BenchmarkFFTDIFReference(b *testing.B) {
	const maxSize = 1 << 20

	pol := make([]mamabear.Element, maxSize)
	pol[0].MustSetRandom()
	for i := 1; i < maxSize; i++ {
		pol[i] = pol[i-1]
	}

	domain := NewDomain(maxSize)

	b.ResetTimer()
	for range b.N {
		domain.FFT(pol, DIF)
	}
}

func BenchmarkFFTDIFReferenceSmall(b *testing.B) {
	const maxSize = 1 << 9

	pol := make([]mamabear.Element, maxSize)
	pol[0].MustSetRandom()
	for i := 1; i < maxSize; i++ {
		pol[i] = pol[i-1]
	}

	domain := NewDomain(maxSize)

	b.ResetTimer()
	for range b.N {
		domain.FFT(pol, DIF)
	}
}

func evaluatePolynomial(pol []mamabear.Element, val mamabear.Element) mamabear.Element {
	var acc, res, tmp mamabear.Element
	res.Set(&pol[0])
	acc.Set(&val)
	for i := 1; i < len(pol); i++ {
		tmp.Mul(&acc, &pol[i])
		res.Add(&res, &tmp)
		acc.Mul(&acc, &val)
	}
	return res
}
