...
func BuildRatioShuffledVectors(numerator, denominator []*Polynomial, beta mamabear.Element, expectedForm Form, domain *fft.Domain) (*Polynomial, error) {
	...
	coeffs := make([]mamabear.Element, n)
	t := make([]mamabear.Element, n)
	coeffs[0].SetOne()
	t[0].SetOne()
	var a, b, c, d mamabear.Element
...
func BuildRatioCopyConstraint(
	...
	parallel.Execute(n-1, func(start, end int) {
		...
		tInv := mamabear.BatchInvert(t[start:end])
		for i := start; i < end; i++ {
...
func getSupportIdentityPermutation(nbCopies int, domain *fft.Domain) []mamabear.Element {
	if nbCopies <= 0 {
		panic("getSupportIdentityPermutation: nbCopies must be positive")
	}

	res := make([]mamabear.Element, uint64(nbCopies)*domain.Cardinality)
