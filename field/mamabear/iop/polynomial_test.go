...
func randomVector(size int) *[]mamabear.Element {
	r := make([]mamabear.Element, size)
	mamabear.Vector(r).MustSetRandom()
	return &r
}
...
func cmpCoefficents(p, q *mamabear.Vector) bool {
	if p.Len() != q.Len() {
		return false
	}
	for i := range p.Len() {
