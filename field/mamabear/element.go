...
// Modulus q =
//
//	q[base10] = 562932773552129
//	q[base16] = 0x1fffc00000001
//
// # Warning
//
...
const (
	...
	Bits  = 49 // number of bits needed to represent a Element
...
// Montgomery radix R = 2^rBits, narrower than the 64-bit word the
// element is stored in. The 64 bits of headroom are what let intermediate
// sums be carried unreduced.
const (
	rBits = 52
...
func (z *Element) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 281466386776065, 0)
...
// LazyAdd returns a + b without reduction; the result is in [0, 2q).
//
// Callers are responsible for reducing before any operation that requires a
// canonical value.
...
// ReduceFast reduces x to [0, 2^1) using the sparse form of q.
//
// Since q = 2^49 − 2^34 + 1 we have 2^49 ≡ 2^34 − 1 (mod q), so writing
// x = x_lo + x_hi · 2^49,
//
//	x ≡ x_lo + x_hi · (2^34 − 1)   (mod q).
//
// The result is NOT fully reduced; call [ConSubP] to bring it into [0, q).
