...
func TestPolynomialEval(t *testing.T) {
	...
	// random value
	var point extensions.E6
	point.MustSetRandom()

	// compute manually f(val)
	var expectedEval, one, den extensions.E6
	var expo big.Int
	one.SetOne()
	expo.SetUint64(20)
...
func TestPolynomialAddConstantInPlace(t *testing.T) {
	...
	// check
	var expectedCoeffs, one extensions.E6
	one.SetOne()
		...
		for l := range domainSize {
			for i := range domainSize {
				...
				if i == l && !y.IsOne() || i != l && !y.IsZero() {
					t.Errorf("domainSize = %d: p_%d(%d) = %s", domainSize, l, i, y.String())
					return false
				}
			}
		}
