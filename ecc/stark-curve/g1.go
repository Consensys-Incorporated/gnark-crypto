...
func (p *G1Jac) DoubleMixed(a *G1Affine) *G1Jac {
	...
	M.Double(&XX).
		Add(&M, &XX) // M = 3*XX
	M.Add(&M, &aCurveCoeff) // M = 3*XX + a (Z=1 for mixed doubling, so a*Z⁴ = a)
	T.Square(&M).
		Sub(&T, &S).
		Sub(&T, &S)
	p.X.Set(&T)
...
func (p *G1Jac) IsOnCurve() bool {
	...
	ZZ.Square(&p.Z)
	{
		...
		aXZ4.Mul(&ZZ, &ZZ).
			Mul(&aXZ4, &p.X).
			Mul(&aXZ4, &aCurveCoeff)
		right.Add(&right, &aXZ4)
	}
...
func (p *g1JacExtended) doubleNegMixed(a *G1Affine) *g1JacExtended {
	...
	M.Double(&t).
		Add(&M, &t) // M = 3*X²
	M.Add(&M, &aCurveCoeff) // M = 3*X² + a (ZZ=1 for mixed doubling)
	p.X.Square(&M)
	t.Double(&S)
	p.X.Sub(&p.X, &t)
	t.Mul(&W, &a.Y)
